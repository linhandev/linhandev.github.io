...
# 符号
一些符号约定，在手写深度学习算法的时候一个很有用的技巧是看矩阵在做乘法加法之类的操作时维度能不能对的上，因此注意看矩阵类型的变量都是什么维度。
...
- $$W^{[l]} \in \mathbb{R}^{n^{[l-1]}_{h} \times n^{[l]}_{h}}$$：计算的时候是
- $$b^{[l]} \in \mathbb{R}$$：
- $$\hat{y} \in \mathbb{R}^{n_{y}}$$：
- $$a^{[L]} \in \mathbb{R}$$：

![scale_drives_dl](/assets/img/_posts/DL/DL-Notes/scale-drives-dl.png)
