...
个人感觉手动的注解主要应该标在变量生命周期的开头附近，之后类型都交给引擎去推。比如 PyLance 如果根据函数返回值认为变量 v 是 int | List[int]，就算你下一行明确写 v: int，PyLance 依旧会坚持他推出来的类型。控制流上的改变才会改变 PyLance 的推理，比如 assert 或者 if 其中一个情况 return。感觉这也很合理，这样才有类似复查的意义，过多的人为介入就成自嗨了。
