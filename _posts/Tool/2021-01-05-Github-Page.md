...
![github-signup](/assets/img/post/2021-01-05-Github-Page/github-signup.png)

Github上的项目可以自己新建也可以复制别人的，就做博客来说直接复制主题项目比较多。Fork的意思就是将别人的一个项目复制一份，存到自己的Github账户下。在每个项目的右上角都有一个Fork按钮

![Fork button](/assets/img/post/2021-01-05-Github-Page/fork.png)
...
![image](/assets/img/post/2021-01-05-Github-Page/152296633-62157681-e911-4fb8-82e6-ced17e103dff.png)
...
![no-page](/assets/img/post/2021-01-05-Github-Page/no-page.png)

Source这里选Markdown所在的Branch

![source](/assets/img/post/2021-01-05-Github-Page/source.png)

后面的文件夹选项是指定在这个Branch下去哪找Markdown文件。如果只在根目录下有一个README.md的话保持默认/(root)就行，如果文档是分成多个文件可以都放到docs文件夹下，选择/docs。

![root](/assets/img/post/2021-01-05-Github-Page/root.png)

保存。

![saved](/assets/img/post/2021-01-05-Github-Page/saved.png)

稍等一会访问上面这个网址就能看到网页了。Pages用一个Enviroment进行静态网站生成和托管，查看进展的方法是首先回到项目主页

![project-home](/assets/img/post/2021-01-05-Github-Page/project-home.png)

在右下角可以看到一个Enviroments，点进那个github-pages

![pages-environment](/assets/img/post/2021-01-05-Github-Page/pages-environment.png)

就可以看到部署的执行情况，刷新几次就会看到一条新的部署记录，点view deployment就会跳转到网页了。

![no theme](/assets/img/post/2021-01-05-Github-Page/no-theme.png)

默认部署没有主题看起来大概比较素，可以在Settings -> Pages -> Theme Chooser -> Choose a theme这里选一个主题。

![choose theme](/assets/img/post/2021-01-05-Github-Page/choose-theme.png)

点击之后会看到主题的预览，点Select Theme选中一个主题。

![theme](/assets/img/post/2021-01-05-Github-Page/theme.png)

这个时候Pages会在代码里创建一个_config.yml文件，记下你选的主题。内容类似这个

![config yml](/assets/img/post/2021-01-05-Github-Page/config-yml.png)
...
![build-fail](/assets/img/post/2021-01-05-Github-Page/build-fail.png)
...
下面就到了这篇的重头戏，用Pages部署个人博客。过程非常简单，选一个主题，用Jekyll或者类似工具（如[hexo](https://hexo.io/)，[Octpress](https://octopress.org)）把Markdown构建成html，之后推到Github项目中就完成了（个人只用过Jekyll所以下文也是针对Jekyll写的，但用其他工具的流程也是一样的）。构建的这一步通常用Github Action，非常方便，搭建的过程10分钟就能搞定。本地构建麻烦一些，但是灵活度高，下文将分别介绍。
...
![fetch-upstream](/assets/img/post/2021-01-05-Github-Page/fetch-upstream.png)

Fork项目之后需要按照 用户名.github.io 格式改项目名，比如我Github用户名是linhandev，那项目名就是 linhandev.github.io。点Settings第一个设置就是。

![rename](/assets/img/post/2021-01-05-Github-Page/rename.png)
...
![action-status](/assets/img/post/2021-01-05-Github-Page/action-status.png)

Action执行过程中是一个黄点，执行成功是一个绿色的对号，如果看到一个红叉就是执行出问题了。执行成功之后点代码左上角的下拉菜单看看是不是多出一个branch

![branch](/assets/img/post/2021-01-05-Github-Page/branch.png)

修改Settings->Pages里的设置，将Source改成这个branch

![change-branch](/assets/img/post/2021-01-05-Github-Page/change-branch.png)

如果commit左边图表是红叉可以看一下Action执行的log来排查具体是什么问题。

- 点项目上面的Action能看到所有的执行

![actions](/assets/img/post/2021-01-05-Github-Page/actions.png)

- 点进其中的一次执行是这样

![action-run](/assets/img/post/2021-01-05-Github-Page/action-run.png)

- 点在那个continus-delivery位置的按钮（不一定叫一个名字）可以看到具体的执行log。

![acton-log](/assets/img/post/2021-01-05-Github-Page/acton-log.png)
...
![page](/assets/img/post/2021-01-05-Github-Page/page.png)

等几分钟网站应该就可以访问了。

![my-blog](/assets/img/post/2021-01-05-Github-Page/my-blog.png)
...
![utterancec-app](/assets/img/post/2021-01-05-Github-Page/utterancec-app.png)

可以只给博客一个项目的权限

![utterances-config](/assets/img/post/2021-01-05-Github-Page/utterances-config.png)

- 安装完成后会打开[utteranc网站](https://utteranc.es/)，按照引导进行设置，先写项目名

![repo-name](/assets/img/post/2021-01-05-Github-Page/repo-name.png)

- 选择每篇文章Issue的格式

![issue-format](/assets/img/post/2021-01-05-Github-Page/issue-format.png)
...
![issue-label](/assets/img/post/2021-01-05-Github-Page/issue-label.png)

- 最后选择一个和主题视觉协调的配色

![utteranc-theme](/assets/img/post/2021-01-05-Github-Page/utteranc-theme.png)

下面能看到一段代码，把它插入到文章模板里正文下面的位置就可以用了。Chirpy的文章模板是在 `_layouts/post.html`，别的主题也都类似。最后的效果

![utteranc-finish](/assets/img/post/2021-01-05-Github-Page/utteranc-finish.png)
...
![sitemap](/assets/img/post/2021-01-05-Github-Page/sitemap.png)
