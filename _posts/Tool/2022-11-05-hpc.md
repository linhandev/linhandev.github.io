...
cp overlay-50G-10M.ext3 paddle-nvidia.ext3
...
conda install paddlepaddle-gpu cudatoolkit=11.6 -c https://mirrors.tuna.tsinghua.edu.cn/anaconda/cloud/Paddle/ -c conda-forge 
...
```shell
srun --cpus-per-task=16 --mem=32GB --time=6:00:00 --gres=gpu:mi50:1 --pty /bin/bash
...
cd /scratch/lh3317/med
export DATASET=organmnist_sagittal
time python train.py --output_root ./resnet50/ --data_name $DATASET --download True >> ./resnet50/$DATASET-res50.log
```
