# New Insights & Discoveries

## Format for New Entries

### KMP Topic: [Title]
- **Date Added**: YYYY-MM-DD
- **Source**: [Where this information came from]
- **Relevance**: [High/Medium/Low] 
- **Tags**: [kmp, multiplatform, android, ios, etc.]
- **Summary**: [Brief description]
- **Details**: [Full explanation]
- **Related**: [Links to related topics in the knowledge base]
- **Action Items**: [What to do with this information]

## Recent Additions

### Example Entry
- **Date Added**: 2026-02-05
- **Topic**: KMP Architecture Patterns
- **Relevance**: High
- **Tags**: kmp, architecture, design-patterns
- **Summary**: Multi-tier architecture with shared business logic layer
- **Details**: Detailed explanation...
- **Related**: ../kmp-foundations/architecture.md
- **Action Items**: Move to kmp-foundations after review

## Pending Review
- [List items that need categorization]